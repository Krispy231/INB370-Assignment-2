...
public class CarPark {
		...
		for (int i = 0; i < vehiclesInQueue.size(); i++) {
			if ((vehiclesInQueue.get(i).isQueued() != true)) {
				...
				throw new VehicleException(
						"The vehicle has been queued for too long");
			} else {
				Vehicle v;
				v = vehiclesInQueue.get(i);
				/*
				 * If the vehicles been in queue for longer than the maximum
				 * queue time then leave the queue and get archived.
				 */
	...
	public void exitQueue(Vehicle v, int exitTime) throws SimulationException,
		...
		if (v.isQueued() == false) {
			...
			throw new VehicleException(
					"Vehicle cannot exit at a negative time.");
	 ...
	 * existing, 91 spaces in car park (P), 84 cars in car park (C), of which 14
	 * are small (S), 7 MotorCycles in car park (M), 48 dissatisfied (D), 176
	...
	public String toString() {
		return "CarPark [count: " + totalVehicleCount + " numCars: "
				+ getNumCars() + " numSmallCars: " + getNumSmallCars()
				+ " numMotorCycles: " + getNumMotorCycles() + " queue: "
				+ numVehiclesInQueue() + " numDissatisfied: "
				+ dissatifiedVehicles.size() + " past: " + past.size() + "]";
	}
